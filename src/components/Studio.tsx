    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-start">
        <div className="reveal">
          <p className="text-xs md:text-sm text-white/70 mb-3">the studio</p>
          <h2 className="hero-title first-letter:uppercase text-white font-medium text-[12vw] md:text-[5vw]">
          ...
          <dl className="mt-12 grid grid-cols-1 sm:grid-cols-2 gap-8">
            {FEATURES.map((feature) => (
              <div key={feature.title}>
                <dt className="text-white text-sm font-semibold mb-2">
                  {feature.title}
                </dt>
                <dd className="text-white/70 text-sm leading-snug">
                  {feature.copy}
                </dd>
              </div>
            ))}
