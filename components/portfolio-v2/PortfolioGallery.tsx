 ...
 * healed shot alongside the fresh one) get an in-tile carousel — same
 * behaviour as components/gallery/GalleryCard.tsx on the production
 * /gallery page, restyled to this route's mono/copper accent system.
 */
...
/**
 * Filterable portfolio grid for the /portfolio-v2 draft — local to this
 * route. Client Component because the artist/category filters and the
 * category rail's scroll-to-center behaviour are local UI state. Filtered
 * tiles deliberately skip the global `.reveal` scroll-in animation (mirrors
 * components/gallery/GalleryGrid.tsx) since that observer only runs once on
 * mount and wouldn't pick up tiles that re-enter the DOM after a filter
 * change.
 */
    ...
    >
      <div className="mx-auto flex w-full max-w-[1360px] flex-col gap-7 px-6 md:gap-9 md:px-10 lg:gap-12">
        ...
        <div className="reveal mt-3 flex flex-col items-center gap-5 border-t border-[var(--gz-border-subtle)] pt-9 md:mt-7 md:gap-6 md:pt-16">
          ...
          <CtaPill as="link" href={V2_SECTIONS.inquire} className="tracking-[0.01em]">
            Inquire now
          </CtaPill>
        </div>
      </div>
