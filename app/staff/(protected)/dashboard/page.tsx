  ...
  return (
    <div>
      <StaffPageHeader
        ...
        action={
          <Link
            ...
            className="rounded-full bg-[var(--primary)] px-5 py-2.5 text-sm font-semibold text-[var(--primary-foreground)] transition-all duration-300 hover:bg-[var(--primary)]/90"
          >
            + create booking
          </Link>
        }
      ...
      <div className="grid grid-cols-1 gap-4 px-4 pb-6 md:px-8 lg:grid-cols-3">
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card)]/60 p-6 md:rounded-2xl lg:col-span-2">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-semibold text-[var(--foreground)]">Recent Inquiries</h2>
            <Link href="/staff/inquiries" className="text-xs text-[var(--primary)] hover:underline">
              View all
            </Link>
          </div>
          <div className="mt-4 flex flex-col divide-y divide-[var(--foreground)]/5">
            {recentInquiries.length === 0 && (
              <p className="py-6 text-center text-sm text-[var(--foreground)]/40">No inquiries yet.</p>
            )}
            {recentInquiries.map((inquiry) => (
              <div key={inquiry.id} className="flex items-center justify-between py-3">
                <div>
                  <p className="text-sm font-medium text-[var(--foreground)]">{inquiry.full_name}</p>
                  <p className="text-xs text-[var(--foreground)]/50">
                    {inquiry.tattoo_type} · {inquiry.preferred_artist}
                  </p>
                </div>
                <span className="text-xs text-[var(--foreground)]/40">{formatDate(inquiry.created_at)}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card)]/60 p-6 md:rounded-2xl">
          <h2 className="text-base font-semibold text-[var(--foreground)]">Payment Status</h2>
          <div className="mt-4 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <StatusBadge status="paid" />
              <span className="text-sm font-semibold text-[var(--foreground)]">{paymentCounts.paid}</span>
            </div>
            <div className="flex items-center justify-between">
              <StatusBadge status="pending" />
              <span className="text-sm font-semibold text-[var(--foreground)]">{paymentCounts.pending}</span>
            </div>
            <div className="flex items-center justify-between">
              <StatusBadge status="failed" />
              <span className="text-sm font-semibold text-[var(--foreground)]">{paymentCounts.failed}</span>
            </div>
            <div className="flex items-center justify-between">
              <StatusBadge status="refunded" />
              <span className="text-sm font-semibold text-[var(--foreground)]">{paymentCounts.refunded}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 pb-10 md:px-8">
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card)]/60 p-6 md:rounded-2xl">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-semibold text-[var(--foreground)]">Recent Bookings</h2>
            <Link href="/staff/bookings" className="text-xs text-[var(--primary)] hover:underline">
              View all
            </Link>
          </div>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-[var(--foreground)]/5">
                {recentBookings.length === 0 && (
                  <tr>
                    <td colSpan={5} className="py-6 text-center text-[var(--foreground)]/40">
                      No bookings yet.
                    </td>
                  </tr>
                )}
                {recentBookings.map((booking) => (
                  <tr key={booking.id}>
                    ...
                    <td className="py-3 pr-4 text-[var(--foreground)]/70">{booking.artistName}</td>
                    <td className="py-3 pr-4">
                      <StatusBadge status={booking.status} />
                    </td>
                    <td className="py-3 pr-4 text-[var(--foreground)]/70">{formatDate(booking.appointmentDate)}</td>
                    <td className="py-3 pr-4 text-[var(--foreground)]/70">
