-- Tracked equivalent of supabase-setup-gallery.sql, parts 3 and 4 (storage
-- bucket + seed data). The gallery_items table and its RLS policies (part
-- 1-2 of that script) were already captured in
-- 20260805013917_remote_schema.sql; this migration closes the remaining
-- gap: the `gallery` Storage bucket, its storage.objects policies, and the
-- 151-row seed of existing portfolio pieces were applied via the Supabase
-- SQL Editor and never had a tracked migration.
--
-- Replayable on a fresh project and safe to re-run against the live
-- project: the bucket insert is ON CONFLICT DO NOTHING, the policies are
-- dropped and recreated, and the seed only runs if gallery_items is empty.
...
drop policy if exists "Staff can upload gallery photos" on storage.objects;
create policy "Staff can upload gallery photos" on storage.objects
  for insert to authenticated with check (bucket_id = 'gallery');

drop policy if exists "Staff can delete gallery photos" on storage.objects;
create policy "Staff can delete gallery photos" on storage.objects
  for delete to authenticated using (bucket_id = 'gallery');

-- Seed data — the 151 entries originally carried over from
-- components/gallery/data.ts. Guarded so replaying this migration never
-- duplicates rows once gallery_items already has data (whether from this
-- seed or from staff edits made since).
insert into gallery_items (piece, category, artist_name, alt, images, display_order)
select * from (values
...
('minimalist piece 12', 'minimalist', 'isaiah recongco', 'minimalist matching four elements symbol tattoos by isaiah recongco — gorillaz tattoo art studio philippines', ARRAY['/images/portfolio/portfolio-164.jpg']::text[], 150))
 as v(piece, category, artist_name, alt, images, display_order)
